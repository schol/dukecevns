...
#include "mixtures.h"
 ...
 // A generic normalization factor for the recoil spectrum
  ...
  //NumericalFlux* monoflux = new NumericalFlux();
  //monoflux->SetFluxFilename("time_integral.dat");
  //monoflux->ReadFluxFile();

  //    PiDAR* monoflux = new PiDAR();

   double monoenergy = j["flux"]["monoenergy"];
   double monoflavor = j["flux"]["monoflavor"];
   
   Monochromatic* monoflux = new Monochromatic();
 std:cout << "Mono energy "<<monoenergy<<std::endl;
   monoflux->SetEnergy(monoenergy);
   monoflux->SetFlavor(monoflavor);

  
  
  double kmax = monoflux->maxEnu();
  std::cout << "kmax "<<kmax << std::endl;
  // Normalize flux for 19.3 m from SNS, per cm^2 per s
  //monoflux->SetNorm(5.e14/(4*M_PI*1950.*1950.));

  double dist = j["distance"];

  double nuspersecperflavor = 1e14;
  monoflux->SetNorm(nuspersecperflavor/(4*M_PI*dist*dist));
  ...
  if (j.find("doosc") != j.end()) {
    ...
    if (doosc == 1) {
      ...
      monoflux->SetOscParam(ua4,dm2,dist);
    }
  }
 ...
 // These are defined in mixtures include
  ...
  std::cout << "kmax, minM "<<kmax<<" "<<minM<<" "<<erecmaxall<<std::endl;
   ...
   for (Erec=erecstart+erecstep;Erec<=erecend; Erec+=erecstep) {
	...
	if (j.find("magmom") != j.end()) {
	 ...
	 if (knu> knumin) {
	   ...
	   drate_mubar_vec += diffxscnvec(knu,M,Erec)*monoflux->fluxval(knu,-2,knustep);
	   drate_tau_vec +=  diffxscnvec(knu,M,Erec)*monoflux->fluxval(knu,3,knustep);
	   drate_taubar_vec +=  diffxscnvec(knu,M,Erec)*monoflux->fluxval(knu,-3,knustep);


	   drate_e_axial += diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,1,knustep);
	   drate_ebar_axial += diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,-1,knustep);
	   drate_mu_axial += diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,2,knustep);
	   drate_mubar_axial += diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,-2,knustep);
	   drate_tau_axial +=  diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,3,knustep);
	   drate_taubar_axial +=  diffxscnaxial(knu,M,Erec)*monoflux->fluxval(knu,-3,knustep);


	   drate_e_interf += diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,1,knustep);
	   drate_ebar_interf += diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,-1,knustep);
	   drate_mu_interf += diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,2,knustep);
	   drate_mubar_interf += diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,-2,knustep);
	   drate_tau_interf +=  diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,3,knustep);
	   drate_taubar_interf += diffxscninterf(knu,M,Erec)*monoflux->fluxval(knu,-3,knustep);

	   drate_e_mag += diffxscnmag(knu,Erec)*monoflux->fluxval(knu,1,knustep);
	   drate_ebar_mag += diffxscnmag(knu,Erec)*monoflux->fluxval(knu,-1,knustep);
	   drate_mu_mag += diffxscnmag(knu,Erec)*monoflux->fluxval(knu,2,knustep);
	   drate_mubar_mag += diffxscnmag(knu,Erec)*monoflux->fluxval(knu,-2,knustep);
	   drate_tau_mag +=  diffxscnmag(knu,Erec)*monoflux->fluxval(knu,3,knustep);
	   drate_taubar_mag += diffxscnmag(knu,Erec)*monoflux->fluxval(knu,-3,knustep);
